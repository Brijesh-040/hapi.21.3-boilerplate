...
exports.plugin = {
    async register(server, options) {
        ...
        if (options.ignoreMethods) {
            for (let i = 0; i < options.ignoreMethods.length; i++) {
                ignoreMethods[options.ignoreMethods[i].toLowerCase()] = true
            }
        }
        
        const ignoreLogTypes = {}
        if (options.ignoreLogTypes) {
            for (let i = 0; i < options.ignoreLogTypes.length; i++) {
                ignoreLogTypes[options.ignoreLogTypes[i].toLowerCase()] = true
            }
        }
        ...
        const getChildBindings = options.getChildBindings ? options.getChildBindings : (request) => ({
            req: request,
        ...
        const dbqueryCompleteMessage = options.customDbqueryCompleteMessageCompleteMessage || function (request, responseTime) { return `[dbquery] ${request.method} ${request.path}` }
        ...
        const requestDbQueryLevel = options.customRequestCompleteLevel || 'info'
        ...
        server.ext('onRequest', (request, h) => {
            mongoose.set('debug', setMongooseDebug(request));
            ...
            // request.logger = logger.child({ requestId: Uuidv4() });
        ...
        const setMongooseDebug = (request) => {
            return async (collectionName, method, query, doc) => {
                const logObj = {
                    type: "DBQUERY",
                    data: {
                        collectionName,
                ...
                if (options.ignoreLogTypes && ignoreLogTypes[logObj.type.toLowerCase()]) {
                    return
                }
                ...
                if (shouldLogRequestComplete(request)) {
                    ...
                    request.logger[requestDbQueryLevel](
                        logObj,
                        dbqueryCompleteMessage(request)
                    )
                }
            };
        };
        ...
        tryAddEvent(server, options, 'on', 'response', function (request) {
            ...
            if (shouldLogRequestComplete(request)) {
                ...
                const logObj = {
                    type: options.logType ? options.logType : undefined,
                    payload: options.logPayload ? request.payload : undefined,
                    response: options.logResponse ? request.response.source : undefined,
                    ...
                    error: options.log4xxResponseErrors && (statusCode >= 400 && statusCode <= 500) ? request.response.source : undefined,
                    isError: (statusCode >= 400 && statusCode <= 500) ? true : false,
        ...
        tryAddEvent(server, options, 'ext', 'onPostStart', async function (s) {
            server.info = { ...server.info, tags: ['ignore'] }
            logger.info(server.info, 'log server started')
        })

        tryAddEvent(server, options, 'ext', 'onPostStop', async function (s) {
            logger.info(server.info, 'log server stopped')
        })
        ...
        function isLoggingIgnored(options, request) {
            ...
            let methodStr = request.method.toLowerCase();
            if (options.ignoreMethods && ignoreMethods[methodStr]) {
                return true
            }
